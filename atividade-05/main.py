...
def generateWebsiteHtml(data):
    ...
    for title, imgsrc in data:
        html += f"<tr><td>{title}</td><td><img src='{imgsrc}' width='100'></td></tr>"
...
def main():
    print("Hello from atividade-05!")

    # open the seeds.txt file and read the urls
    ...
    for url in urls:
        ...
        for img in soup.find_all("img"):
            ...
            if not imgsrc.startswith("http") and not imgsrc.startswith("data"):
                imgsrc = urljoin(url, imgsrc)
            if imgsrc:
                print("src: ", imgsrc, "\n")
                data.append((title, imgsrc))
                break
